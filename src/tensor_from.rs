use crate::{dtype::Dtype, shape::Const, tensor::Tensor};
...
impl<const D1: usize, T: Dtype> From<Vec<T>> for Tensor<T, (Const<D1>,)> {
    fn from(value: Vec<T>) -> Self {
        assert_eq!(value.len(), D1);
        unsafe { Self::from_vec_unchecked(value) }
    }
}
impl<const D1: usize, const D2: usize, T: Dtype> From<Vec<T>>
...
impl<T: Dtype, const D1: usize> From<[T; D1]> for Tensor<T, (Const<D1>,)> {
    fn from(value: [T; D1]) -> Self {
        unsafe { Self::from_vec_unchecked(value.into()) }
    }
}

impl<T: Dtype, const D1: usize, const D2: usize> From<[[T; D2]; D1]>
    for Tensor<T, (Const<D1>, Const<D2>)>
{
